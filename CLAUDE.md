...
8. **Read-side cache freshness is now the ONLY cache-bust signal (Bug #1538).** The retired versioned/pointer design changed a shard's PATH on every publish, so the per-worker, path-keyed `HNSWIndexCache` was invalidated for free. A fixed path removes that side effect: nothing about the key changes when the file's CONTENT does, and each uvicorn worker (on each node) holds its own entry. Bug #1538 measured the consequence on the 3-node cluster -- ~58% of post-refresh temporal queries served the pre-refresh graph, still ~58% twelve minutes later, never self-healing. Two independent defects, both fixed:
   ...
   - **An unverifiable freshness check drops the entry (Messi #13).** A `None` fingerprint (its pre-load stat failed) or a stat that fails on a HIT means we CANNOT CONFIRM the cached graph still matches disk. `_index_file_changed` logs a WARNING and returns True -- it must never return False, which would silently treat "could not check" as "unchanged" and can serve a superseded graph indefinitely with no signal. Dropping is safe: the reload runs the caller's loader, and a loader finding no index returns None, which `get_or_load` already handles explicitly (EVO-64244 Facet 1). Self-correcting -- at most one extra load, never a loop. `test_missing_index_file_does_not_crash_on_hit` (`test_hnsw_negative_cache_evo64244.py`) was updated accordingly: the no-crash property it protects is unchanged, but it now asserts the reload instead of the old serve-the-unverified-entry behavior.
