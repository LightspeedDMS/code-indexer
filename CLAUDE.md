...
**Temporal lineage lookup reads the SHARED metadata store (Bug #1533)**: item 4's read seam resolves the golden alias by reading activated-repo metadata, and WHICH STORE it reads is itself a correctness decision. `temporal_worker.py` used to construct its OWN `ActivatedRepoManager(data_dir=...)` outside the DI chain -- never wired to the shared PostgreSQL backend, so every read hit NODE-LOCAL metadata. On a cluster the activation row lives in the shared `activated_repos` table and the node-local store is EMPTY, so the lookup returned `None`, which legitimately means "no golden lineage", which produced the all-None context, which read the activation's CoW clone -- correctly empty of temporal data since this bug. Confirmed live on clustered staging: activated-repo temporal queries returned HTTP 200 with `total_results: 0`, no error surfaced, and the SAME standalone manager's `.golden_repo_manager` raised `GoldenRepoNotFoundError` on every temporal query (swallowed by `load_golden_temporal_config`'s fail-open, silently degrading embedder selection). Invisible on solo/SQLite, where the node-local store IS the shared store. Binding rules now:

- **The worker never selects its own store.** `_resolve_lineage_repo_manager` (`temporal_worker.py`) prefers the INJECTED `activated_repo_manager`, and in postgres/cluster mode raises `TemporalLineageStoreUnavailableError` rather than read node-local metadata -- both when the kwarg is absent and when the injected manager is not fully wired. Cluster mode must FAIL LOUD: "I looked in the wrong store" is not the same answer as "there is no lineage", the same principle as item 4's fail-closed rule. Solo/SQLite and pure CLI keep the legacy standalone construction (still honoring `CIDX_SERVER_DATA_DIR`, Bug #1517) because there the node-local store IS the real store.
- **BOTH shared stores are required; the predicate is PLURAL and checks CAPABILITY, not presence.** `ActivatedRepoManager.uses_shared_metadata_stores()` returns True only when the activation pool actually exposes `.connection` AND `golden_repo_manager.has_shared_metadata_backend()` -- which asks the CURRENT backend whether it IS the shared store (`is_shared_backend`, declared `True` on `GoldenRepoMetadataPostgresBackend` and `False` on `GoldenRepoMetadataSqliteBackend`). INJECTED is not the same as SHARED: an earlier version returned True for any injected backend, so an accidentally-injected node-local SQLite backend passed the cluster guard and would have read node-local state -- the exact bug class this guard exists to close. Checking the activation pool alone is likewise insufficient: a half-wired manager resolves the lineage correctly and then still fails the golden lookup with `GoldenRepoNotFoundError`. There is no singular `uses_shared_metadata_store()` and no `has_injected_metadata_backend()`; do not reintroduce either.
- **BOTH doors must inject it.** `activated_repo_manager` is threaded from `app.state.activated_repo_manager` through `execute_live_temporal_search` into `submit_job(...)`'s worker kwargs, exactly like `query_tracker` -- never into `TemporalWorkerInput`/the dedup signature. MCP (`mcp/handlers/search.py` via `_utils._get_activated_repo_manager()`) and REST (`routers/inline_query.py`) are both wired; Bug #1482 is the precedent for why one door wired is not "wired". AST-guarded per call site by `tests/unit/server/services/test_temporal_doors_pass_activated_repo_manager_1533.py`. Any fake `worker_fn` test seam must declare the kwarg too.
- **`is_postgres_storage_mode()`** (`server/utils/registry_factory.py`) is the single authority for the `app.state.storage_mode == "postgres"` probe -- never reimplement the `sys.modules` app.state introspection.
- **Live-PostgreSQL tests must not be able to destroy real data.** `tests/unit/server/storage/postgres/test_temporal_worker_lineage_live_pg_1533.py` creates a uniquely-named private SCHEMA per test, reaches its tables through a `search_path` option on the DSN, drops ONLY that schema, and FAILS (never silently skips) unless the `TEST_POSTGRES_DSN` database name FULLY matches a disposable format (`re.fullmatch` against `_DISPOSABLE_DB_NAME_REGEX`, digits-only optional suffix). Substring containment is NOT acceptable and must never be reintroduced: `marker in db_name` accepted `production_cidx_test` and `cidx_test_prod`. The guard is unit-tested WITHOUT any database (`TestDisposableDatabaseGuard`), so it is exercised on every run rather than only where PostgreSQL happens to be configured. It never creates or drops a production-shaped table in `public`, guarded by `test_private_schema_isolation_leaves_public_schema_intact` (compares EVERY `public` table's name+OID -- ordinary and partitioned -- across the fixture run, catching create, drop and drop-and-recreate; it is READ-ONLY and must stay that way, an earlier sentinel-table version wrote into `public` and contradicted this very claim). Older live-PG modules that `DROP TABLE IF EXISTS <fixed production name>` against whatever the DSN points at (e.g. `test_migration_runner.py`'s `isolated_schema` on `schema_migrations`) are a KNOWN, still-outstanding hazard -- do not copy them.

-> Detail: docs/architecture-invariants.md#temporal-worker-lineage-store-selection-bug-1533
