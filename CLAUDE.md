...
- **BOTH shared stores are required; the predicate is PLURAL.** `ActivatedRepoManager.uses_shared_metadata_stores()` returns True only when the activation connection pool is set AND `golden_repo_manager.has_injected_metadata_backend()` (the DI chain supplied `BackendRegistry.golden_repo_metadata`; `GoldenRepoManager` records `_metadata_backend_injected` at construction). Checking the activation pool alone is NOT sufficient -- a half-wired manager resolves the lineage correctly and then still fails the golden lookup with `GoldenRepoNotFoundError`, reproducing this bug's second symptom. There is no singular `uses_shared_metadata_store()`; do not reintroduce one.
