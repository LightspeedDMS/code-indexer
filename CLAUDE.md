...
**Story #1167 (Workers Un-Pin)**: `_ensure_workers_config()` reads `config.workers` via `ServerConfigManager(server_dir_path=str(_cidx_data_dir)).load_config()` (same bootstrap-config idiom as all sibling `_ensure_*` methods) and writes `--workers {worker_count}` into the ExecStart line. Uses `max(1, getattr(config, "workers", 1) or 1)` to guard misconfigured zero/negative values. Workers=1 produces byte-identical output to the old hardcoded behavior. Idempotency guard is VALUE-AWARE (see Bug #1183 below) -- NOT presence-only. Single-writer invariant: `_ensure_workers_config` is the ONLY method that writes a `--workers` token to the unit file -- `restart_server()` and `HealthWatchdog._restart_server()` both call `systemctl restart` on the existing unit without modifying it. Web UI: `"workers"` is in `RESTART_REQUIRED_FIELDS` (routes.py); the Server Settings display table shows "Uvicorn Workers" with restart-required note; the edit form has a number input (1-64); validation rejects outside-range and non-integer values. Backend: `workers` was already in `BOOTSTRAP_KEYS` and `_update_server_setting` already mapped it -- no backend changes needed.
