...
- **The worker never selects its own store.** `_resolve_lineage_repo_manager` (`temporal_worker.py`) prefers the INJECTED `activated_repo_manager`, and in postgres/cluster mode raises `TemporalLineageStoreUnavailableError` rather than read node-local metadata -- neither when the kwarg is absent nor when the injected manager lacks its cluster pool (`ActivatedRepoManager.uses_shared_metadata_store()`, the public predicate over `_pool`). Cluster mode must FAIL LOUD: "I looked in the wrong store" is not the same answer as "there is no lineage", the same principle as item 4's fail-closed rule. Solo/SQLite and pure CLI keep the legacy standalone construction (still honoring `CIDX_SERVER_DATA_DIR`, Bug #1517) because there the node-local store IS the real store.
