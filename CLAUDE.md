...
Alias pointers written while that mechanism was briefly enabled may still exist on some installations; they are inert. Reading pre-#1529 relocated data is not supported by any current code path -- the fixed-path root is the only location consulted.
