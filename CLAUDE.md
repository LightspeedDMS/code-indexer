...
- **regex_search-aligned param names (v10.3.x)**: `pattern` (was `driver_regex`), `max_results` (was `max_files`). Plus content-driver params `path`, `case_sensitive`, `multiline`, `pcre2`, `context_lines` (0..10).
...
- `max_results` cap surfaces `partial=True` / `max_files_reached=True` in the result; job-level timeout takes precedence (`partial=True` / `timeout=True`).
