...
- **Even a ping-and-wait-one-cycle protocol is NOT sufficient for agents doing SSH/multi-node/log-capture work** (confirmed by a second false-positive kill, same session, 2026-07-18: an agent running log capture across 3 staging cluster nodes and coordinating a real front-door trigger produced zero output-file activity for ~20 minutes AND never answered a direct ping — not because it was stuck, but because the whole multi-step operation (SSH session setup + log tailing + waiting on a real subprocess launch across nodes) doesn't flush intermediate output until a step boundary completes). For this class of task, treat 20+ minutes of silence plus an unanswered ping as inconclusive, not confirmed-dead — prefer an even longer wait, or ask the agent up front to report intermediate checkpoints more frequently, over killing.
...
- If you do decide to kill despite ambiguity, save the agent's last-known status report (if it sent one) so a restart can skip re-doing completed investigation/setup work — the kill notification's `result` field often shows exactly what it was mid-way through.
