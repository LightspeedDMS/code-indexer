...
- [feedback_opus_arbiter_of_codex_nitpicking.md](feedback_opus_arbiter_of_codex_nitpicking.md) - After 2-3 Codex REJECT rounds on the same fix, dispatch Opus to judge materiality vs nitpicking BEFORE auto-escalating another round — Bug #1548 ran 10 rounds without this check
...
- [project_config_default_flip_is_inert.md](project_config_default_flip_is_inert.md) - Changing a dataclass default in config_manager.py is INERT on existing deployments (persisted JSON blob merged OVER defaults); needs a one-time promotion in _merge_runtime_config keyed on a marker's PRESENCE, not its value
