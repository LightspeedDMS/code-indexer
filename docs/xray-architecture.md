...
- **Phase 2, graph mode (`analyze_graph`, cross-file)**: a different execution mode from single-file `xray_search`. It requires both `fn collect_facts` (runs per file to collect auxiliary evidence) and `fn analyze_graph` (reduces the completed cross-file graph); a graph evaluator must not define `evaluate_node`. Graph extraction currently supports Java and Kotlin (`.kt`/`.kts`), which bind to each other in both directions; Kotlin is extracted at bind levels 0-2 only, with no receiver-type substrate -- Kotlin hops never carry `RECEIVER_TYPE_MATCH`, which costs evidence quality (confidence ranking) but NOT an edge for an ordinary qualified call (Bug #1920, verified end-to-end): `apply_receiver_type_narrowing` is permanently tag-only in this binder, so an instance-qualified call (`g.helper(x)`) binds by name+arity exactly like a type-qualified one and a target reachable only that way is not reported definitely dead -- see `kotlin.rs`'s module doc for the full evidence trail. Calls made through essentially every operator convention (`a + b`/`-`/`*`/`/`/`%`, `a < b`/`<=`/`>`/`>=`, `a == b`/`!=`, `m[k]`/`m[k] = v` read/write, unary/postfix `!f`/`-x`/`+x`/`x++`/`--x`, range/containment `a..b`/`a..<b`/`x in y`/`x !in y`, and non-indexed compound assignment `x += y` etc., which over-binds BOTH the `*Assign` and plain candidate names) ARE extracted; a compound assignment onto an INDEXED target (`m[k] += v`) and the `invoke` convention are still not extracted. Always check `fact_graph_complete` and the degradation counters before treating an empty `findings` list as a verified negative. See "Graph Mode: Node and Edge Model" below for what becomes a node, what becomes an edge, and what is invisible. For the `UserFact`, `GraphResult`, `ReduceFinding`, graph-handle, and completeness contracts, fetch the full tool documentation: `get_file_content(repository_alias='code-indexer-global', file_path='src/code_indexer/server/mcp/tool_docs/search/analyze_graph.md')`.
...
`_truncate_xray_batch_result(result, payload_cache)` — thin wrapper delegating to the shared `xray_truncation.truncate_result_fields(result, payload_cache, ["matches", "errors", "evaluation_errors"])` (Bug #1928; also used by `xray.py`'s `_truncate_xray_result` and `xray_graph.py`'s `_truncate_graph_result`):

- `payload_cache is None`: returns a BOUNDED degrade response (`cache_unavailable: true`; still `truncated: true` with a bounded inline prefix if the raw result exceeds the fallback character budget, never the full unbounded arrays) — round 3 (Codex P3) replaced the earlier "return the full result unchanged" behavior.
- Serializes combined matches + errors + evaluation_errors as JSON; if the size is <= the server's single payload character budget (`payload_max_fetch_size_chars`, default 5000 chars): returns the original result dict unchanged (`cache_handle: None`, `truncated: false`).
- Otherwise: packs WHOLE, unmodified entries into successive pages (never splitting/shrinking an entry; one that alone exceeds the budget gets its own oversized page; packing is O(n), tracking a running serialized-size delta per entry rather than re-serializing the whole page each time — round 3, Opus P3). Pre-generates every page handle plus a prefix-discriminated (`xray-pv1-`) manifest handle, then writes ALL of them (pages + the `pages-v1` manifest referencing those handles) as ONE atomic batch via `PayloadCache.store_batch_with_keys()` — one shared timestamp/expiry, failures propagate (round 3, Codex P1; the earlier design wrote pages via `store_batch()` then the manifest via a SEPARATE `store()` call, which could leave orphaned pages or a dangling manifest on partial failure). A write failure raises `PageSetStoreError`, caught here and surfaced as `{"success": false, "error": "cache_store_failed", ...}` rather than a `cache_handle` pointing at data that was never durably written. On success, returns a truncated dict whose `matches`/`errors`/`evaluation_errors` are a character-budget-driven WHOLE-entry prefix (never a fixed count of 3) plus `cache_handle`, `has_more`, `truncated`, `total_pages`, `inline_entry_truncated` (set only when the single first entry alone exceeded the budget and was adaptively shrunk for this inline response), `fetch_tool_hint`. `_job_fn` already runs inside the `BackgroundJobManager` worker thread pool, off the event loop, so no additional `anyio.to_thread.run_sync` offload is needed at this call site.
