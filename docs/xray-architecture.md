...
### Candidate-admission contract: arity narrows to empty on known evidence; receiver-type TAGS ONLY (Bug #1898, scope split, epic #1906)

**`apply_arity_narrowing` narrows to EXACTLY the matches, all the way down to EMPTY, once the evidence it depends on is KNOWN** -- `arg_count: Some(n)` and the candidate's own `param_count` known too, unambiguous closed-world facts about how many arguments a call site passed and a declaration accepts. Before Bug #1898 (fixed in epic #1906 P1) this pass treated an empty match as "unsafe to narrow" and silently kept the entire bare-name pool instead -- for a call whose real target is outside the repository, that fallback fabricated an edge to a same-named but structurally unrelated in-repo candidate. A single external-receiver call like `connection.close()` resolving against an unrelated in-repo `close(1 params)` method is the production shape this produced; `callers_of`/`callees_of` were the most visibly affected consumers (see epic #1906's P0 baseline). Across all four review rounds of this bug, `apply_arity_narrowing` was never once implicated -- every finding traced to receiver-type narrowing instead.

**`apply_receiver_type_narrowing` is TAG-ONLY: it may set the `RECEIVER_TYPE_MATCH` reason bit on a candidate whose enclosing type matches the resolved receiver type (or one of its transitive supertypes), but it NEVER removes a candidate from the set -- not on an empty match, not on a non-empty match, under `Positive` evidence or `Advisory`.** This is the end state of four straight review rounds, all rejected for variations for the SAME root defect (a hard-narrow deleting a candidate on receiver-type evidence that turned out not to be closed-world):

- **Round 1**: hard-emptied on EVERY resolved receiver type unconditionally. Rejected: a coincidental same-bare-name collision between the real binding and an unrelated in-repo declaration deleted genuine edges and flipped live private methods to a false definitely-dead verdict.
- **Round 2**: taught `resolve_receiver_type` to reject two more failure shapes -- the literal string `"var"`/a bare generic-type-parameter name (pseudo-types, never a narrowable class), and six binding forms (enhanced-for/try-with-resources/catch/lambda parameters, inner-class/inherited field access) that previously had NO typed-name evidence at all and fell through to a fallback that could misresolve them into an unrelated in-repo type.
- **Round 3**: rejected for the SAME shape recurring against `instanceof` pattern variables and locals declared inside a static/instance initializer block or a record compact constructor -- another enumerated binding form the hard filter had never been taught about.
- **Round 4**: replaced enumeration with a structural rule -- `resolve_receiver_type` returns a `receiver::ReceiverEvidence` tagged `Positive` (an actual `TypedNameRecord` lookup hit, or the call's own enclosing type -- never a coincidental collision, in theory) or `Advisory` (either open-world fallback substrate), and only `Positive` evidence could hard-empty a zero match.
- **Round 4's OWN review** (`.analysis/1898-review-rounds/round4-findings.md`) found this tiered design unsound TWO further ways, neither closable by yet another enumerated case: (1) `Positive` is not closed-world either -- `receiver::FileTypedNames` keys locals by `(enclosing_method, name)` while Java scopes by BLOCK, so two same-named locals in different blocks of one method collide, last-write-wins, and the hard filter can delete the wrong one's real candidate; (2) NEITHER evidence tier ever guarded narrowing to a non-empty WRONG subset -- only the "zero match" branch was ever hardened, leaving "matches something, but the wrong something" completely unguarded on both tiers.

Four rounds of enumerating binding forms and evidence tiers could not converge on a safe hard-empty contract for a heuristic Java extractor with no classpath/generics resolution. Hard receiver-type narrowing (exclusive binding to a statically-qualified type; discriminating a same-arity external receiver from an in-repo same-named method) is deferred whole-cloth to follow-up issue #1910, which can design a genuinely closed-world evidence substrate from scratch, rather than patching this one a fifth time. Until then, receiver-type evidence contributes only the `RECEIVER_TYPE_MATCH` reason bit (and hence `Confidence::ReceiverType`) -- real signal for a caller that wants it, never a candidate-set exclusion. The `Positive`/`Advisory` tiering itself is KEPT (not ripped out): it still drives the reason bits and is still consulted by the AC4 Level 5 unique-name-in-repo shortcut (`try_unique_name_shortcut`, below) and by #1910's eventual design.

- **`try_unique_name_shortcut` still declines its shortcut on a POSITIVE receiver-type mismatch, unaffected by this split.** This is a genuinely different decision from the narrowing pass above: whether a SOLE candidate may claim `UNIQUE_NAME_IN_REPO`/`Confidence::Exact` without running the rest of the pipeline is an ADMISSION choice, not a DELETION of an already-built candidate set. Declining the shortcut simply falls through to the (now tag-only) full pipeline, which keeps the candidate regardless -- so this gate can never itself cause the false-dead-code regressions the narrowing hard-filter did.
- **Incomplete supertype evidence**: `apply_receiver_type_narrowing` still skips TAGGING entirely when the receiver type's own supertype evidence is recorded incomplete (a `superclass`/`implements` clause that exists syntactically but could not be resolved to a name, or a same-bare-name collision this extractor cannot locally disambiguate -- see #1882/#1883), consulting the same substrate `apply_super_class_narrowing` uses for this exact reason. Since tagging never removes anything either way, an incomplete `allowed` set can only under-tag (miss a genuine `RECEIVER_TYPE_MATCH`), never over-tag or over-exclude.

**`apply_same_class_or_super_narrowing` (the AC3 evidence path for an unqualified/`this` call resolving against the caller's own type and its supertypes) deliberately KEPT its pre-#1898 soft "no match -> keep the pool" fallback.** Its `allowed` set (`{enclosing_type} U supertypes_of(enclosing_type)`) covers only DIRECT inheritance evidence, but Java's real unqualified-call scope is strictly larger: an inner/anonymous/static-nested/local class's bare call can resolve against ANY lexically enclosing type (not just its own declared supertypes), and a static-imported bare call resolves against the imported class -- neither relationship is inheritance, so `allowed` cannot see either one. Hard-narrowing this pass end-to-end on real javac-valid source flipped `is_definitely_dead_code` from `Some(false)` to a FALSE `Some(true)` on live private targets called from an enclosing/anonymous/static-nested/local class, or reached only via a static import -- exactly the outcome epic #1786 declared structurally impossible. Widening `allowed` to the caller's full lexical nest (the same `type_nesting`/`top_level_of` substrate D2's private-visibility filter already uses) plus exempting `reasons::STATIC_IMPORT` candidates is the real long-term fix and is intentionally deferred to #1910 (second scope item); see `apply_same_class_or_super_narrowing`'s own doc comment in `narrowing.rs` for the full rationale. **Known consequence**: `super.m()` with an implicit `Object` superclass (#1885) does NOT close under this epic -- `apply_super_class_narrowing` (below) is a genuinely separate pass from same-class-or-super and still early-returns on `allowed.is_empty()` for exactly that reason, unchanged by #1898.

`super`-class narrowing itself (D3, an explicit `super.m()`/`super(...)` call) already implemented the hard-empty "empty means empty, except on incomplete evidence" contract before #1898 -- see the paragraph above -- and needed no change. It is unaffected by the receiver-type scope split: it narrows on the enclosing type's OWN recorded supertypes, never on a resolved receiver type.

**Statically-qualified calls on a known in-repo type (AC3) still resolve their receiver even when it names a TYPE rather than a variable, independent of the scope split.** `receiver::resolve_receiver_type`'s `Identifier` arm, when the identifier is not a local/field/parameter with declared-type evidence IN THE SAME FILE, first tries the repo-wide unambiguous field-type index (a genuine field access this per-file lookup missed), and only when THAT has no answer falls back to `TypeIndex::is_known_type_name` -- so `TimeUtil.parse(x)` resolves its receiver to `"TimeUtil"` itself (a known in-repo type, from the same `type_nesting` substrate `top_level_of` reads) instead of leaving it unresolved; both are Advisory-tier. Resolving the receiver at all is unaffected by the scope split -- what changed is that this resolution now only ever TAGS `TimeUtil.parse`, it no longer excludes `ParserA.parse`/`ParserB.parse` from the same call's candidate set (that exclusion is exactly #1910's scope). **The `is_known_type_name` fallback is itself gated (round-2 hardening): it never fires when the identifier is ALSO a known field name anywhere in the repo** -- an earlier version of this fallback fired unconditionally on any lookup miss, which misresolved a genuine field access into an unrelated in-repo type whenever one happened to share the identifier's bare name.

**Observability (round 4, epic #1906 mandate item 3)**: `RepoIndexResult::narrowed_to_zero_count` (`repo_index.rs`) and `--build-graph`'s JSON (`BuildGraphResult::narrowed_to_zero_count`, `xray-cli/src/main.rs`) surface how many references, across the whole build, had a real same-named declaration somewhere in the repo but were narrowed all the way to zero final candidates -- deliberately distinct from an ordinary out-of-repo reference (bare-name pool also empty, never counted). Since the scope split, this counter is driven by `apply_arity_narrowing` alone (receiver-type narrowing can no longer produce a zero-candidate outcome by itself). Three rounds of narrowing regressions survived a green 531-test suite because nothing counted this; a future run over a mixed/unusual codebase can now see the number move without waiting for a symptom report. Sibling story #1897 wires it into `analyze_graph`'s own completeness reporting.
