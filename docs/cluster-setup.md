...
**CRITICAL -- NFS mount options**: All NFS mounts MUST use `soft,timeo=30,retrans=3` (NOT `hard`). The `hard` option causes server threads to enter uninterruptible D-state (`nfs_wait_bit_killable`) on any NFS blip, permanently blocking dep-map analysis and other background jobs. The `soft` option returns EIO after timeout instead of blocking forever. The mount MUST also include `nolock`: this cluster's shared NFSv4 mount has been observed suffering periodic server-side lock-manager state loss (`dmesg` showing `NFS: <server>: lost N locks`), which directly caused a SQLite `disk I/O error` during a golden-repo refresh -- SQLite's `chunks.db` writes depend on real OS-level byte-range locks, and it is the NFS lock-manager's state for those locks (not write contention) that periodically breaks down. `nolock` makes locking client-side-only, so a transient server-side lock-manager hiccup can no longer interrupt an in-progress write. This is safe specifically because this project's own `WriteLockManager` already serializes writers per-repo at the application layer -- `nolock`'s only real risk (genuine concurrent writers with no external coordination) does not apply here. `nolock` and `soft`/`hard` are orthogonal options; do not confuse the two. Example fstab entry:

```
192.168.60.23:/path/to/export /mnt/cow-storage nfs4 soft,timeo=30,retrans=3,nolock,_netdev 0 0
```
