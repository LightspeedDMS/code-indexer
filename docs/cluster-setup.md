...
**CRITICAL -- NFS mount options**: All NFS mounts MUST use `soft,timeo=30,retrans=3` (NOT `hard`). The `hard` option causes server threads to enter uninterruptible D-state (`nfs_wait_bit_killable`) on any NFS blip, permanently blocking dep-map analysis and other background jobs. The `soft` option returns EIO after timeout instead of blocking forever. The mount MUST also use **NFSv3 with `nolock`** (`vers=3,nolock` -- not NFSv4): this cluster's shared mount was originally NFSv4.1 and observed suffering periodic server-side lock-manager state loss (`dmesg` showing `NFS: <server>: lost N locks`), which directly caused a SQLite `disk I/O error` during a golden-repo refresh -- SQLite's `chunks.db` writes depend on real OS-level byte-range locks, and it is the NFS lock-manager's state for those locks (not write contention) that periodically breaks down. `nolock` makes locking client-side-only via the NFSv3-only NLM protocol, so a transient server-side lock-manager hiccup can no longer interrupt an in-progress write. `nolock`/`local_lock` was first tried on the existing NFSv4.1 mount alone and empirically confirmed to have **zero effect** there -- NFSv4 integrates locking into the protocol's own OPEN/LOCK state machine rather than the separate NLM protocol `nolock` controls, so NFSv4 has no client-side-only locking mode at all. The mount was therefore downgraded to NFSv3, where `nolock` is a real, effective option. This is safe specifically because this project's own `WriteLockManager` already serializes writers per-repo at the application layer -- `nolock`'s only real risk (genuine concurrent writers with no external coordination) does not apply here. `nolock`/`vers=3` and `soft`/`hard` are orthogonal options; do not confuse the two. Example fstab entry:

```
<storage-node-ip>:/path/to/export /mnt/cow-storage nfs vers=3,nolock,soft,timeo=30,retrans=3,_netdev 0 0
```

**Exception -- the golden-repos mount**: When using the CoW Storage Daemon, the golden-repos directory itself is a separate NFS mount that MUST use `vers=3,nolock,hard` (`hard` not `soft` -- both mounts now share NFSv3 with `nolock`, but differ on the soft/hard setting). Git's `index-pack` step during golden-repo indexing mmaps its pack and index files and fails on a `soft` mount. This is a deliberate difference from the clone mount above; apply `hard` specifically to the golden-repos mount. See [Golden Repository Shared Storage (CoW Daemon Dev Clusters)](#golden-repository-shared-storage-cow-daemon-dev-clusters) below for the full configuration and rationale.
