...
2. **BOTH shared stores are required, and the predicate is PLURAL.** `ActivatedRepoManager.uses_shared_metadata_stores()` returns True only when the activation connection pool is set AND `golden_repo_manager.has_injected_metadata_backend()` (i.e. the DI chain supplied `BackendRegistry.golden_repo_metadata`). Checking the activation pool alone is insufficient: a half-wired manager resolves the lineage correctly and then still fails the golden lookup with `GoldenRepoNotFoundError`, reproducing the second symptom above. `GoldenRepoManager` records `_metadata_backend_injected` at construction (True when a backend is injected, False when it self-constructs `GoldenRepoMetadataSqliteBackend`).
