...
"""
        ...
        images = extractor.extract_images(malformed_content, test_file, multimodal_repo_path)
...
"""
    ...
    def test_edge_case_files_dont_crash_parsers(self, multimodal_repo_path):
        ...
        for edge_file in edge_cases_dir.glob("*"):
            ...
            if edge_file.suffix in [".md", ".markdown"]:
                images = markdown_extractor.extract_images(content, edge_file, multimodal_repo_path)
                assert isinstance(images, list), f"Markdown parser crashed on {edge_file.name}"
                
            elif edge_file.suffix in [".html", ".htmx", ".htm"]:
                images = html_extractor.extract_images(content, edge_file, multimodal_repo_path)
                assert isinstance(images, list), f"HTML parser crashed on {edge_file.name}"
    ...
    def test_text_that_looks_like_image_syntax(self, multimodal_repo_path):
        ...
        md_file = multimodal_repo_path / "docs" / "edge-cases" / "text-looks-like-image.md"
