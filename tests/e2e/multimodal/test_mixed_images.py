...
class TestMixedImages:
    ...
    def test_mixed_valid_invalid_extraction(self, multimodal_repo_path):
        ...
        md_file = multimodal_repo_path / "docs" / "edge-cases" / "mixed-valid-invalid.md"
    ...
    def test_invalid_images_have_correct_skip_reasons(self, multimodal_repo_path):
        ...
        assert len(invalid_results) == 3, f"Expected 3 invalid images, got {len(invalid_results)}"
        ...
        unsupported_results = [r for r in invalid_results if r.skip_reason == "unsupported_format"]
        
        assert len(missing_results) == 1, f"Expected 1 missing image, got {len(missing_results)}"
        assert len(remote_results) == 1, f"Expected 1 remote URL, got {len(remote_results)}"
        assert len(unsupported_results) == 1, f"Expected 1 unsupported format, got {len(unsupported_results)}"
    ...
    def test_all_images_in_validation_results(self, multimodal_repo_path):
        ...
        assert invalid_count == 3, f"Expected 3 invalid in all_results, got {invalid_count}"
