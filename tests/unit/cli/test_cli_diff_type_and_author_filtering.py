...
class TestCLIDiffTypeAndAuthorFiltering(unittest.TestCase):
    ...
    def test_cli_passes_diff_type_to_temporal_service(self):
        ...
        This test verifies the wiring exists by checking that
        _execute_temporal_fusion (the current temporal query entry point,
        aliased from execute_temporal_query_with_fusion) is called with the
        diff_types parameter from the CLI options.
        """
        ...
        # Verify that _execute_temporal_fusion is called with diff_types parameter
        assert "_execute_temporal_fusion(" in source, (
            "_execute_temporal_fusion call not found in query command"
        )
        assert "diff_types=" in source, (
            "diff_types parameter not passed to _execute_temporal_fusion"
        )
    ...
    def test_cli_passes_author_to_temporal_service(self):
        ...
        # Verify that _execute_temporal_fusion is called with author parameter
        assert "_execute_temporal_fusion(" in source, (
            "_execute_temporal_fusion call not found in query command"
        )
        assert "author=" in source, (
            "author parameter not passed to _execute_temporal_fusion"
        )
