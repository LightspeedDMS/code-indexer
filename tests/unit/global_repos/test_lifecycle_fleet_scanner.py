...
def test_find_broken_or_missing_broken_cases(
    ...
    if should_be_flagged:
        ...
        assert alias not in broken, f"Expected {alias!r} NOT to be flagged (valid metadata)"
...
def test_find_broken_or_missing_returns_empty_for_empty_input(golden_repos_dir: Path) -> None:
...
def test_find_broken_or_missing_skips_cidx_meta_self_alias(golden_repos_dir: Path) -> None:
    ...
    fm_yaml = yaml.dump(_VALID_FRONTMATTER, default_flow_style=False, allow_unicode=True)
