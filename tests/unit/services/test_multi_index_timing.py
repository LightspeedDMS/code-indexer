...
class TestMultiIndexTiming:
    ...
    def test_single_index_timing_structure(self, service, mock_vector_store):
        ...
        assert "multimodal_index_ms" not in timing, "Should not have multimodal_index_ms when no multimodal"
        assert "parallel_multi_index_ms" not in timing, "Should not have parallel_multi_index_ms for single index"
        assert "merge_deduplicate_ms" not in timing, "Should not have merge timing for single index"
        assert "code_timed_out" in timing, "Should have code_timed_out flag"
        assert timing["code_timed_out"] is False, "code_timed_out should be False on success"
    ...
    def test_multi_index_timing_structure(
        ...
        multimodal_dir = mock_project_root / ".code-indexer" / "index" / "voyage-multimodal-3"
        ...
        assert "parallel_multi_index_ms" in timing, "Should have parallel_multi_index_ms"
        assert "code_index_ms" in timing, "Should have code_index_ms"
        ...
        assert timing["code_timed_out"] is False, "code_timed_out should be False on success"
        assert timing["multimodal_timed_out"] is False, "multimodal_timed_out should be False on success"
    ...
    def test_parallel_timing_is_max_not_sum(
        ...
        assert code_time >= 20 and code_time < 40, f"code_index_ms {code_time}ms should be ~23ms (sleep time)"
        assert multimodal_time >= 38 and multimodal_time < 60, f"multimodal_index_ms {multimodal_time}ms should be ~41ms (sleep time)"
        ...
        assert parallel_time >= 40, f"Parallel time {parallel_time}ms should be >= 40ms (approx max simulated time)"
        assert parallel_time < 70, f"Parallel time {parallel_time}ms should be < 70ms (not sum)"
    ...
    def test_merge_deduplicate_timing_captured(
        ...
        def search_side_effect(*args, **kwargs):
            ...
            if collection == "voyage-multimodal-3":
                return (
                    [{"payload": {"path": "file2.md", "chunk_offset": 0}, "score": 0.85}],
                    {"embedding_ms": 20, "hnsw_search_ms": 15}
                )
            else:
                return (
                    [{"payload": {"path": "file1.py", "chunk_offset": 0}, "score": 0.9}],
                    {"embedding_ms": 23, "hnsw_search_ms": 15}
                )
        ...
        assert "merge_deduplicate_ms" in timing, "Should capture merge/deduplicate timing"
        assert timing["merge_deduplicate_ms"] >= 0, "Merge timing should be non-negative"
        # Merge should be relatively fast (<50ms typically)
        assert timing["merge_deduplicate_ms"] < 100, "Merge timing should be reasonable (<100ms)"
    ...
    def test_timeout_flags_set_on_code_timeout(
        ...
        assert timing["code_timed_out"] is True, "code_timed_out should be True when code query times out"
        assert timing["multimodal_timed_out"] is False, "multimodal_timed_out should be False (multimodal succeeded)"
    ...
    def test_timeout_flags_set_on_multimodal_timeout(
        ...
        assert timing["code_timed_out"] is False, "code_timed_out should be False (code succeeded)"
        assert timing["multimodal_timed_out"] is True, "multimodal_timed_out should be True when multimodal query times out"
    ...
    def test_individual_index_timing_captured(
        ...
        assert timing["multimodal_index_ms"] >= 0, "multimodal_index_ms should be non-negative"
        # Both should be numeric (wall-clock measurements)
        assert isinstance(timing["code_index_ms"], (int, float)), "code_index_ms should be numeric"
        assert isinstance(timing["multimodal_index_ms"], (int, float)), "multimodal_index_ms should be numeric"
