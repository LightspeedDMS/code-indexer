"""Tests for C2 fix (Bug #1078): Cohere get_embedding must not sleep internally.

Symmetric to test_voyage_single_attempt_retry_false.py. get_embedding is the
QUERY path — it must call _make_sync_request(retry=False) so that:
...
_FAKE_KEY = "test-cohere-key-c2"

# Minimal successful Cohere embed response (v2 API format).
# embed-v4.0 default_dimension is 1536 (see CohereConfig in config.py).
...
class TestCohereGetEmbeddingUsesRetryFalse:
    """get_embedding must call _make_sync_request(..., retry=False)."""
...
class TestCohereMAkeSyncRequestRetryFalse:
    ...
    def test_single_attempt_on_500_no_sleep(self):
        ...
        def fake_post(*args: Any, **kwargs: Any) -> None:
            nonlocal call_count
            call_count += 1
            req = httpx.Request("POST", "https://api.cohere.com/v1/embed")
    ...
    def test_single_attempt_success_returns_result(self):
        """On success with retry=False: returns the response dict normally."""
        client = _make_client()
...
class TestCohereMAkeSyncRequestRetryTrue:
...
class TestCohereMAkeSyncRequestRetryTrue429Retries:
    ...
    Indexing batches now silently drop rate-limited chunks. This class validates
