...
class TestPayloadCacheSQLiteWAL:
    ...
    def test_database_created_with_wal_mode(self, temp_db_path):
    ...
    def test_table_schema_created(self, temp_db_path):
        ...
        try:
            ...
            # Check columns
            cursor = conn.execute("PRAGMA table_info(payload_cache)")
    ...
    def test_index_created_on_created_at(self, temp_db_path):
...
class TestPayloadCacheStoreRetrieve:
    ...
    def test_store_returns_uuid4_handle(self, cache):
    ...
    def test_store_saves_content_and_metadata(self, cache, temp_db_path):
    ...
    def test_retrieve_page_0_returns_first_chunk(self, cache):
    ...
    def test_retrieve_page_1_returns_second_chunk(self, cache):
        """Test retrieve() page 1 returns chars 5000-9999."""
        content = "A" * 5000 + "B" * 5000
        handle = cache.store(content)

        result = cache.retrieve(handle, page=1)
    ...
    def test_retrieve_small_content_single_page(self, cache):
    ...
    def test_retrieve_invalid_handle_raises_error(self, cache):
        ...
        with pytest.raises(CacheNotFoundError) as exc_info:
            cache.retrieve("invalid-handle", page=0)

        assert "invalid-handle" in str(exc_info.value)

    def test_retrieve_page_out_of_range_raises_error(self, cache):
