...
class TestPayloadCacheTruncation:
    ...
    def test_truncate_result_large_content(self, cache):
        """Test truncate_result for content larger than preview_size_chars."""
        large_content = "X" * 500
        result = cache.truncate_result(large_content)
    ...
    def test_truncate_result_empty_content(self, cache):
        """Test truncate_result for empty content."""
        result = cache.truncate_result("")

        assert result["content"] == ""
        assert result["has_more"] is False
        assert result["cache_handle"] is None

    def test_truncate_result_cached_handle_is_retrievable(self, cache):
        ...
        retrieved = cache.retrieve(handle, page=0)
