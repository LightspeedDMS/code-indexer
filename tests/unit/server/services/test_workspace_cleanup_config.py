...
class TestWorkspaceCleanupConfigAC1:
    ...
    def test_default_retention_period_is_7_days(self, tmp_path):
        ...
        Note: Story #15 moved scip_workspace_retention_days into ScipConfig.
        """
        ...
        # Story #15: scip_workspace_retention_days moved to scip_config
        assert config.scip_config is not None
        assert hasattr(config.scip_config, "scip_workspace_retention_days")
        assert config.scip_config.scip_workspace_retention_days == 7
    ...
    def test_retention_period_via_config_file(self, tmp_path):
        ...
        # Write config with retention period in new nested format (Story #15)
        config_file.write_text('{"scip_config": {"scip_workspace_retention_days": 14}}')
        ...
        assert config.scip_config.scip_workspace_retention_days == 14
    ...
    def test_retention_period_via_environment_variable(self, tmp_path, monkeypatch):
        ...
        # Write config with retention period 14 days (new nested format - Story #15)
        config_file.write_text('{"scip_config": {"scip_workspace_retention_days": 14}}')
        ...
        # Story #15: scip_workspace_retention_days moved to scip_config
        assert config is not None
        assert config.scip_config.scip_workspace_retention_days == 21
    ...
    def test_retention_period_minimum_validation(self, tmp_path):
        ...
        config = ServerConfig(server_dir=server_dir)
        # Story #15: scip_workspace_retention_days moved to scip_config
        config.scip_config.scip_workspace_retention_days = 0
    ...
    def test_retention_period_maximum_validation(self, tmp_path):
        ...
        config = ServerConfig(server_dir=server_dir)
        # Story #15: scip_workspace_retention_days moved to scip_config
        config.scip_config.scip_workspace_retention_days = 366
    ...
    def test_retention_period_persists_to_config_file(self, tmp_path):
        ...
        # Story #15: scip_workspace_retention_days moved to scip_config
        config.scip_config.scip_workspace_retention_days = 14
    ...
    def test_retention_period_type_validation(self, tmp_path):
        ...
        # Story #15: scip_workspace_retention_days moved to scip_config
        config.scip_config.scip_workspace_retention_days = 7
        manager = ServerConfigManager(server_dir)
        manager.validate_config(config)  # Should not raise

        # Should reject string (or convert if that's the behavior)
        with pytest.raises((TypeError, ValueError)):
            config.scip_config.scip_workspace_retention_days = "not an integer"
            manager.validate_config(config)
