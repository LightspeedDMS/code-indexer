...
def test_create_memory_returns_id_and_content_hash(
    memories_dir, lock_manager, rate_limiter
):
    svc, _s, _d = _build_service(
        memories_dir, lock_manager, rate_limiter, id_factory=lambda: FIXED_UUID
...
def test_create_memory_respects_max_summary_chars(
    memories_dir, lock_manager, rate_limiter
):
    svc, _s, _d = _build_service(
...
def test_create_memory_triggers_refresh_direct_acquire(
    memories_dir, lock_manager, rate_limiter
):
...
def test_create_memory_piggyback_signals_debouncer_only(
    memories_dir, lock_manager, rate_limiter
):
    svc, scheduler, debouncer = _build_service(
        memories_dir,
...
def test_create_memory_never_triggers_on_schema_failure(
    memories_dir, lock_manager, rate_limiter
):
...
def test_edit_memory_happy_path(memories_dir, lock_manager, rate_limiter):
    ...
    edit_result = svc.edit_memory(
        memory_id, edit_payload, original_hash, username="bob"
    )
...
def test_edit_memory_stale_hash_raises_stale_content_error(
    memories_dir, lock_manager, rate_limiter
):
    ...
    ids = _make_counter_id_factory()
    svc, _s, _d, memory_id, real_hash = _create_one(
        memories_dir, lock_manager, rate_limiter, ids
    )

    with pytest.raises(StaleContentError) as exc_info:
        svc.edit_memory(
            memory_id, _valid_create_payload(), "wrong_hash_value", username="alice"
        )
...
def test_edit_memory_not_found(memories_dir, lock_manager, rate_limiter):
    ...
    with pytest.raises(NotFoundError):
        svc.edit_memory(
            "nonexistent00001", _valid_create_payload(), "any_hash", username="alice"
        )


@pytest.mark.parametrize(
    "immutable_field,bad_value",
    [
        ("id", "different-id"),
        ("created_by", "hacker"),
...
def test_edit_memory_stale_does_not_trigger_refresh(
    memories_dir, lock_manager, rate_limiter
):
...
def test_delete_memory_stale_hash_raises(memories_dir, lock_manager, rate_limiter):
    ...
    ids = _make_counter_id_factory()
    svc, _s, _d, memory_id, _hash = _create_one(
        memories_dir, lock_manager, rate_limiter, ids
    )
    file_path = memories_dir / f"{memory_id}.md"
...
def test_per_memory_lock_released_on_exception(
    memories_dir, lock_manager, rate_limiter
):
...
def test_coarse_lock_released_on_exception_direct_acquire(
    memories_dir, lock_manager, rate_limiter
):
...
def test_coarse_lock_not_released_on_piggyback(
    memories_dir, lock_manager, rate_limiter
):
...
def test_owner_name_includes_hostname_and_username(
    memories_dir, lock_manager, rate_limiter
):
