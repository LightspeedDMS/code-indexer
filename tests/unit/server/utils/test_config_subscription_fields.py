...
class TestSubscriptionFieldsPersistence:
    ...
    def test_save_and_load_llm_creds_provider_url(self, tmp_path):
        ...
        config.claude_integration_config.llm_creds_provider_url = (
            "http://creds.example.com:8080"
        )
        config_manager.save_config(config)

        loaded = config_manager.load_config()
        assert (
            loaded.claude_integration_config.llm_creds_provider_url
            == "http://creds.example.com:8080"
        )
    ...
    def test_save_and_load_llm_creds_provider_api_key(self, tmp_path):
        ...
        loaded = config_manager.load_config()
        assert (
            loaded.claude_integration_config.llm_creds_provider_api_key
            == "secret-provider-key"
        )
    ...
    def test_save_and_load_llm_creds_provider_consumer_id(self, tmp_path):
        ...
        loaded = config_manager.load_config()
        assert (
            loaded.claude_integration_config.llm_creds_provider_consumer_id
            == "my-custom-consumer"
        )
