...
are mocked; internal omni helpers are NOT mocked so the real branching logic is exercised.
"""
...
def _invoke_handler(handler_name: str, repo_alias: Any) -> Dict[str, Any]:
    ...
    Does NOT mock any internal helpers (_submit_xray_search_omni /
    _submit_xray_explore_omni) so the real branching logic is exercised.
    """
...
def test_single_element_string_array_returns_single_repo_shape(
    handler_name: str,
) -> None:
    """repository_alias=["cidx-meta-global"] -> {job_id: ...}, not {job_ids: [...]}."""
    data = _invoke_handler(handler_name, ["cidx-meta-global"])
    assert "job_id" in data, (
        f"[{handler_name}] Expected 'job_id' key, got: {list(data.keys())}"
    )
    assert data["job_id"] == _FAKE_JOB_ID
    assert "job_ids" not in data


@pytest.mark.parametrize("handler_name", _HANDLER_NAMES)
def test_single_element_json_encoded_array_returns_single_repo_shape(
    handler_name: str,
) -> None:
...
@pytest.mark.parametrize("handler_name", _HANDLER_NAMES)
def test_two_element_array_returns_multi_repo_shape(handler_name: str) -> None:
    ...
    """
    data = _invoke_handler(handler_name, ["repo-a", "repo-b"])
...
@pytest.mark.parametrize("handler_name", _HANDLER_NAMES)
def test_string_alias_returns_single_repo_shape(handler_name: str) -> None:
