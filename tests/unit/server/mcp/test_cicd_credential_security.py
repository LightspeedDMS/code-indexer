...
class TestDeriveForgeHost:
    ...
    def test_strips_https_protocol(self):
        assert _derive_forge_host("https://gitlab.example.com", "gitlab") == "gitlab.example.com"

    def test_strips_http_protocol(self):
        assert _derive_forge_host("http://gitlab.example.com", "gitlab") == "gitlab.example.com"
...
class TestResolveCicdProjectAccess:
    ...
    def test_unregistered_project_is_allowed(self):
        ...
        registry = MagicMock()
        registry.list_global_repos.return_value = []  # No repos registered
    ...
    def test_no_access_filtering_service_allows_all(self):
        ...
        registry = self._make_registry_with_repo(
...
class TestGitLabListPipelinesGroupAccess:
    ...
    async def test_access_check_before_token_resolution(self):
        ...
        with patch(
            ...
            "code_indexer.server.mcp.handlers._resolve_cicd_read_token",
            side_effect=lambda *a, **kw: token_resolver_called.append(True) or "token",
        ):
            await handle_gitlab_ci_list_pipelines(args, user)

        # Token resolver must NOT have been called
        assert len(token_resolver_called) == 0, "Token resolver called despite access denial"
...
class TestWriteHandlersUsePerUserPat:
    ...
    async def test_gitlab_retry_pipeline_fails_without_personal_credential(self):
        ...
        with patch(
            ...
            return_value=(None, "Configure personal git credential for gitlab.com to perform write operations. Use configure_git_credential tool."),
        ):
    ...
    async def test_github_retry_run_fails_without_personal_credential(self):
        ...
        with patch(
            ...
            "code_indexer.server.mcp.handlers._resolve_cicd_write_token",
            return_value=(None, "Configure personal git credential for github.com to perform write operations. Use configure_git_credential tool."),
        ):
    ...
    async def test_gitlab_retry_pipeline_uses_personal_pat_not_global(self):
        ...
        ) as mock_write_token, patch(
            "code_indexer.server.clients.gitlab_ci_client.GitLabCIClient"
        ) as MockClient:
            mock_instance = MagicMock()
            mock_instance.retry_pipeline = AsyncMock(return_value={"id": 200, "status": "running"})
            mock_instance.last_rate_limit = None
            MockClient.return_value = mock_instance
...
class TestAuditTrailInWriteHandlers:
    ...
    async def test_gitlab_retry_pipeline_logs_audit_entry(self, caplog):
        ...
        with patch(
            ...
            "code_indexer.server.clients.gitlab_ci_client.GitLabCIClient"
        ) as MockClient, caplog.at_level(logging.INFO, logger="code_indexer.server.mcp.handlers"):
            mock_instance = MagicMock()
        ...
        assert any(
            "alice" in r.message and ("retry" in r.message.lower() or "myproject" in r.message)
            for r in audit_logs
    ...
    async def test_gitlab_cancel_pipeline_logs_audit_entry(self, caplog):
        ...
        with patch(
            ...
            mock_instance = MagicMock()
            mock_instance.cancel_pipeline = AsyncMock(return_value={"status": "cancelled"})
            mock_instance.last_rate_limit = None
            MockClient.return_value = mock_instance
        ...
        audit_logs = [r for r in caplog.records if r.levelno == logging.INFO]
        assert any(
            "alice" in r.message and ("cancel" in r.message.lower() or "myproject" in r.message)
            for r in audit_logs
        ), f"Audit log missing. Records: {[r.message for r in audit_logs]}"
    ...
    async def test_github_retry_run_logs_audit_entry(self, caplog):
        ...
        audit_logs = [r for r in caplog.records if r.levelno == logging.INFO]
        assert any(
            "bob" in r.message and ("retry" in r.message.lower() or "myrepo" in r.message)
            for r in audit_logs
        ), f"Audit log missing. Records: {[r.message for r in audit_logs]}"
    ...
    async def test_github_cancel_run_logs_audit_entry(self, caplog):
        ...
        audit_logs = [r for r in caplog.records if r.levelno == logging.INFO]
        assert any(
            "charlie" in r.message and ("cancel" in r.message.lower() or "myrepo" in r.message)
            for r in audit_logs
        ), f"Audit log missing. Records: {[r.message for r in audit_logs]}"
    ...
    async def test_audit_log_includes_correlation_id(self, caplog):
        ...
        audit_logs = [r for r in caplog.records if r.levelno == logging.INFO]
        assert any(
            getattr(r, "correlation_id", None) is not None
            for r in audit_logs
...
class TestAllHandlersCoverage:
    ...
    def _patch_write_handler_deps(self, project_denied: bool = False):
        ...
        return [
            ...
            patch(
                "code_indexer.server.mcp.handlers._resolve_cicd_write_token",
                return_value=(None, "no cred") if project_denied else ("test_token", None),
            ),
        ]
