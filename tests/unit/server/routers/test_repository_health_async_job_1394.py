"""
Bug #1394: async job endpoint for repository health checks.

GET /api/repositories/{alias}/health previously ran the full health check
synchronously inside the request/response cycle -- on a golden repo with
dozens of quarterly temporal shards this exceeds the reverse-proxy timeout
(HTTP 504). This suite covers the new
POST /api/repositories/{repo_alias}/health/check?force_refresh=bool endpoint,
which submits a background job instead and returns 202 + job_id immediately.

Also includes a smoke test proving the GET endpoint's rewire onto the shared
compute_repository_health() aggregator preserves identical external
behavior (Bug #1394 section 3).

repository_health.py's `_get_golden_repo_manager` / `_get_activated_repo_manager`
/ `_get_background_job_manager` helpers read from the GLOBAL
`code_indexer.server.app.app.state` singleton (not the request-scoped
`request.app.state`), so these are patched directly rather than set as
attributes on a locally-built FastAPI test app.

Real BackgroundJobManager (in-memory, no sqlite) -- no mocking of job
submission/execution. Real on-disk hnswlib indexes for the aggregation
assertions.
"""
...
def _make_golden_repo_manager(clone_path: Path, known_alias: str = "myrepo"):
    """Mock golden_repo_manager resolving known_alias -> clone_path."""
    mock_grm = MagicMock()

    def get_golden_repo(alias: str):
        return object() if alias == known_alias else None

    def get_actual_repo_path(alias: str):
        assert alias == known_alias
        return str(clone_path)

    mock_grm.get_golden_repo.side_effect = get_golden_repo
    mock_grm.get_actual_repo_path.side_effect = get_actual_repo_path
    return mock_grm
...
class _AppUnderTest:
    """Bundles a minimal FastAPI app + TestClient with the repository_health
    router's manager-lookup helpers patched to return test doubles.

    Use as a context manager: `with _AppUnderTest(...) as client:`.
    """
    ...
    def __enter__(self) -> TestClient:
        ...
        mock_grm = _make_golden_repo_manager(self._clone_path, self._known_alias)
...
class TestPostHealthCheckSubmitsJob:
    def test_returns_202_with_job_id_string(self, tmp_path: Path, real_job_manager):
        with _AppUnderTest(tmp_path, real_job_manager) as client:
            response = client.post("/api/repositories/myrepo/health/check")
    ...
    def test_repo_not_found_returns_404(self, tmp_path: Path, real_job_manager):
        with _AppUnderTest(tmp_path, real_job_manager, known_alias="myrepo") as client:
            response = client.post("/api/repositories/nonexistent-repo/health/check")
...
class TestPostHealthCheckDuplicateJob:
    def test_duplicate_pending_job_returns_409(self, tmp_path: Path, real_job_manager):
        # Pre-populate an in-flight job for the same (operation_type, repo_alias)
        # key that submit_job's legacy in-process dedup guard checks.
        existing_job_id = "existing-health-check-job"
        job = BackgroundJob(
...
class TestWorkerResultShapeMatchesRepositoryHealthResult:
    ...
    def test_full_flow_against_real_hnsw_indexes(
        ...
        with _AppUnderTest(tmp_path, real_job_manager, mount_job_routes=True) as client:
            submit_resp = client.post(
                "/api/repositories/myrepo/health/check",
...
class TestGetHealthEndpointRemoved:
    """Bug #1453: the synchronous GET /{repo_alias}/health endpoint is REMOVED
    entirely (it could block a request thread past the reverse-proxy timeout
    on repositories with many temporal shards -- the same hazard #1394 fixed
    for the async POST sibling, which stays live). No route should match this
    path/method combination anymore."""
    ...
    def test_get_health_route_no_longer_exists(self, tmp_path: Path, real_job_manager):
        ...
        with _AppUnderTest(tmp_path, real_job_manager) as client:
            response = client.get(
                "/api/repositories/myrepo/health", params={"force_refresh": "true"}
            )
