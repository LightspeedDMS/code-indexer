...
class TestDeleteEndpointErrorHandling:
    ...
    def test_delete_nonexistent_repository_returns_404(
        ...
        # Mock GoldenRepoError (repository not found) on the REAL manager
        # instance captured as a closure parameter by Story #409's route
        # extraction (register_inline_routes) -- patching the app module's
        # global name never reaches that closure.
        monkeypatch.setattr(
            client.app.state.golden_repo_manager,
            "remove_golden_repo",
            MagicMock(
                side_effect=GoldenRepoError("Golden repository 'nonexistent' not found")
            ),
        )
    ...
    def test_delete_permission_error_returns_500(
        ...
        # Mock GitOperationError (permission/cleanup failure) on the REAL
        # manager instance (see test_delete_nonexistent_repository_returns_404
        # for why patching src.code_indexer.server.app.golden_repo_manager
        # does not work).
        monkeypatch.setattr(
            client.app.state.golden_repo_manager,
            "remove_golden_repo",
    ...
    def test_delete_filesystem_error_returns_500(
        ...
        # Mock GitOperationError (filesystem failure) on the REAL manager
        # instance (closure-capture class, see above).
        monkeypatch.setattr(
            client.app.state.golden_repo_manager,
            "remove_golden_repo",
            MagicMock(
    ...
    def test_delete_successful_returns_204(self, client, auth_headers, monkeypatch):
        ...
        # Mock successful removal on the REAL manager instance (closure-
        # capture class, see above).
        monkeypatch.setattr(
            client.app.state.golden_repo_manager,
            "remove_golden_repo",
            MagicMock(
    ...
    def test_delete_generic_error_returns_500(self, client, auth_headers):
        """Test DELETE with generic error returns HTTP 500."""
        # Mock unexpected error on the REAL manager instance captured as a
        # closure parameter by Story #409's route extraction -- patching
        # src.code_indexer.server.app.golden_repo_manager never reaches it.
        with patch.object(
            client.app.state.golden_repo_manager,
            "remove_golden_repo",
    ...
    def test_delete_error_categorization_scenarios(self, client, auth_headers):
        ...
        for exception, expected_status, expected_text in test_cases:
            # Patch the REAL manager instance captured as a closure
            # parameter by Story #409's route extraction -- patching
            # src.code_indexer.server.app.golden_repo_manager never
            # reaches it.
            with patch.object(
                client.app.state.golden_repo_manager,
                "remove_golden_repo",
