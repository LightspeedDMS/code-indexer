    ...
    def test_article_page_shows_real_views_from_db(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # Front matter views now appears as "Salesforce Views" alongside DB "Views"
                # Verify the Salesforce Views label is present for the front matter value
                assert ">Salesforce Views<" in resp.text
                # The DB-driven Views count must be separate and different from front matter 999
                assert ">Views<" in resp.text
            finally:
                try:
                    os.unlink(db_path)
