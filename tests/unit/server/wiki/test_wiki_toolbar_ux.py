...
def _js_path() -> Path:
    ...
    project_root = here.parent.parent.parent.parent
    return (
        project_root / "src" / "code_indexer" / "server" / "wiki" / "static" / "wiki.js"
    )
...
class TestTemplateStructure:
    ...
    def test_sidebar_toggle_button_has_compact_class(self):
        ...
        sidebar_toggle_lines = [line for line in lines if 'id="sidebar-toggle"' in line]
        assert len(sidebar_toggle_lines) >= 1, "No line containing sidebar-toggle found"
        assert (
            "toolbar-btn-compact" in sidebar_toggle_lines[0]
    ...
    def test_theme_toggle_button_has_compact_class(self):
        ...
        lines = html.splitlines()
        theme_toggle_lines = [line for line in lines if 'id="theme-toggle"' in line]
        assert len(theme_toggle_lines) >= 1, "No line containing theme-toggle found"
        assert (
            "toolbar-btn-compact" in theme_toggle_lines[0]
    ...
    def test_drag_handle_element_exists(self):
        ...
        html = _read_template()
        assert (
            'id="wiki-sidebar-handle"' in html
    ...
    def test_drag_handle_has_correct_class(self):
        ...
        html = _read_template()
        lines = html.splitlines()
        handle_lines = [line for line in lines if "wiki-sidebar-handle" in line]
    ...
    def test_drag_handle_is_inside_or_adjacent_to_sidebar(self):
        ...
        assert handle_pos > sidebar_start, "Handle must come after sidebar opening tag"

    def test_wiki_home_link_still_present(self):
        """Regression: Wiki Home link must still be present in the toolbar."""
        html = _read_template()
        assert (
            "toolbar-home" in html
        ), "Wiki Home link with class toolbar-home must remain"
...
class TestCSSValidation:
    ...
    def test_toolbar_btn_compact_uses_flex_display(self):
        ...
        compact_block = css[compact_idx:block_end]
        assert (
            "display: flex" in compact_block or "display:flex" in compact_block
    ...
    def test_wiki_sidebar_handle_uses_flex_stretch(self):
        ...
        handle_block = css[handle_idx:block_end]
        assert (
            "align-self: stretch" in handle_block
            or "align-self:stretch" in handle_block
    ...
    def test_wiki_sidebar_does_not_have_resize_horizontal(self):
        ...
        assert (
            "resize: horizontal" not in sidebar_block
    ...
    def test_wiki_sidebar_has_min_width_180px(self):
        ...
        block_end = css.find("}", sidebar_idx)
        sidebar_block = css[sidebar_idx:block_end]
        assert "180px" in sidebar_block, ".wiki-sidebar must keep min-width: 180px"
    ...
    def test_wiki_sidebar_resizing_class_exists(self):
        ...
        css = _read_css()
        assert (
            ".wiki-sidebar.resizing" in css or ".resizing" in css
    ...
    def test_wiki_sidebar_has_position_for_handle_absolute_positioning(self):
        ...
        sidebar_block = css[sidebar_idx:block_end]
        has_relative = (
            "position: relative" in sidebar_block
            or "position:relative" in sidebar_block
        )
        has_sticky = (
            "position: sticky" in sidebar_block or "position:sticky" in sidebar_block
        )
...
class TestJavaScriptValidation:
    ...
    def test_init_sidebar_resize_function_exists(self):
        ...
        js = _read_js()
        assert (
            "function initSidebarResize" in js
    ...
    def test_window_resize_listener_recalculates_constraints(self):
        ...
        js = _read_js()
        assert (
            "window.addEventListener" in js
        ), "window.addEventListener must be present in wiki.js"
        assert (
            '"resize"' in js or "'resize'" in js
    ...
    def test_init_sidebar_resize_called_after_init_sidebar_toggle(self):
        ...
        assert (
            resize_call_idx > toggle_call_idx
