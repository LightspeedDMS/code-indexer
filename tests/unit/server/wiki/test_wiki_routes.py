...
def _make_app(
    ...
    wiki_static = (
        Path(__file__).parent / "../../../../src/code_indexer/server/wiki/static"
    )
    if wiki_static.exists():
        app.mount(
            "/wiki/_static",
            StaticFiles(directory=str(wiki_static.resolve())),
            name="wiki_static",
        )
...
class TestAccessControl:
    def test_disabled_wiki_returns_404(self):
        app = _make_app(
            _make_user("alice"), wiki_enabled=False, user_accessible_repos={"test-repo"}
        )
...
class TestSecurity:
    ...
    def test_value_error_alias_returns_404(self):
        ...
        app.state.golden_repo_manager.get_actual_repo_path.side_effect = ValueError(
            "bad"
        )
        client = TestClient(app, raise_server_exceptions=False)
