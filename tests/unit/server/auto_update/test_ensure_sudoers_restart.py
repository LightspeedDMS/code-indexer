...
"""
        ...
        with patch.object(Path, "exists", return_value=True):
            ...
            with patch.object(Path, "read_text", return_value=service_content):
                # Only service file is read via Path.read_text
                with patch("subprocess.run") as mock_run:
                    # sudo cat returns the expected rule (file exists and has correct content)
                    mock_run.return_value = MagicMock(
                        returncode=0,
                        stdout=expected_rule + "\n",  # Include newline as cat would
                    )
...
"""
        ...
        with patch.object(Path, "exists", return_value=True):
            ...
            with patch.object(Path, "read_text", return_value=service_content):
                with patch("subprocess.run") as mock_run:
                    # First call: sudo cat (file doesn't exist)
                    # Second call: sudo tee (create file)
                    # Third call: sudo chmod 0440
                    # Fourth call: sudo visudo -c -f (validate)
                    mock_run.side_effect = [
                        MagicMock(returncode=1),  # cat failure (file doesn't exist)
        ...
        # Verify tee call (now at index [1])
...
"""
        ...
        with patch.object(Path, "exists", return_value=True):
            ...
            with patch.object(Path, "read_text", return_value=service_content):
                with patch("subprocess.run") as mock_run:
                    # First call: sudo cat (file doesn't exist)
                    # Second call: sudo tee (success)
                    # Third call: sudo chmod 0440 (success)
                    # Fourth call: sudo visudo -c -f (FAILURE)
                    # Fifth call: sudo rm -f (cleanup)
                    mock_run.side_effect = [
                        MagicMock(returncode=1),  # cat failure (file doesn't exist)
                        MagicMock(returncode=0),  # tee success
                        MagicMock(returncode=0),  # chmod success
        ...
        # Verify cleanup call happened (now at index [4])
        assert mock_run.call_count == 5
        cleanup_call = mock_run.call_args_list[4]
...
"""
        ...
        assert result is False
        # Should have tried cat and tee, but not chmod or visudo
