...
class TestConfigManagerSocketPath:
    ...
    def test_get_socket_path_uses_new_helper(self, mock_create_mapping, mock_generate):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mock_create_mapping.assert_called_once_with(
                config_path.parent.parent, mock_socket_path
            )
            assert socket_path == mock_socket_path

    @patch("code_indexer.daemon.socket_helper.generate_socket_path")
    @patch("code_indexer.daemon.socket_helper.create_mapping_file")
    def test_get_socket_path_respects_socket_mode(
        self, mock_create_mapping, mock_generate
    ):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config_path.write_text(
                '{"daemon": {"enabled": true, "socket_mode": "user"}}'
            )
    ...
    @patch("code_indexer.daemon.socket_helper.create_mapping_file")
    def test_get_socket_path_uses_custom_socket_base(
        self, mock_create_mapping, mock_hash, mock_ensure
    ):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config_path.parent.mkdir(parents=True)
            config_path.write_text(
                '{"daemon": {"enabled": true, "socket_base": "/custom/socket/dir"}}'
            )
    ...
    @patch("code_indexer.daemon.socket_helper.generate_socket_path")
    @patch("code_indexer.daemon.socket_helper.create_mapping_file")
    def test_get_socket_path_creates_mapping_file(
        self, mock_create_mapping, mock_generate
    ):
    ...
    @patch("code_indexer.daemon.socket_helper.generate_repo_hash")
    @patch("code_indexer.daemon.socket_helper.create_mapping_file")
    def test_socket_base_with_user_mode(
        self, mock_create_mapping, mock_hash, mock_ensure
    ):
