...
use crate::graph::extract::local_index::{
    Declaration, DeclarationKind as DK, LocalIndex, MethodOwnerRecord,
...
// `is_definite_type_qualifier` and `resolve_dotted_qualifier_type` (with
// their own unit tests) moved to the sibling module `receiver_type_
// qualifier.rs`/`receiver_type_qualifier_tests.rs` (Messi Rule 6,
// anti-file-bloat split, rework of the #1931 fix) -- both functions are
// specific to type-qualifier hard-narrowing, distinct from this file's
// CORE AC1/AC2 receiver-type-resolution substrate.
