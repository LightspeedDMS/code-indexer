...
- set_limit(new): clamps to [K_MIN, K_MAX] and notify_all() so parked acquirers
  re-evaluate the predicate on growth. Shrinking NEVER kills in-flight work — it
  only makes future acquires wait until releases bring in_flight below the new
  limit.
...
class ResizableLimiter:
    ...
    def __init__(self, initial: int) -> None:
        ...
        self._limit: int = _clamp_limit(initial)
    ...
    def set_limit(self, new_limit: int) -> None:
        """Set the concurrency limit, clamped to [K_MIN, K_MAX].
        ...
        """
        with self._cond:
            self._limit = _clamp_limit(new_limit)
