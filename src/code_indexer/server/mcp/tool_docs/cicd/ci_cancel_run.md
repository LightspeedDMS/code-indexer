...
tl_dr: Cancel a running CI/CD workflow or pipeline, auto-detecting the forge from the remote URL.
slim_description: "Cancel a running CI/CD workflow run or pipeline by run_id, auto-detecting GitHub Actions or GitLab CI. Requires personal git credential."
inputSchema:
  type: object
  properties:
    repository_alias:
    ...
    run_id:
      type: integer
      description: Workflow run ID (GitHub) or pipeline ID (GitLab) to cancel
    forge:
      type: string
      enum:
      - auto
