...
**Kotlin scope limits, stated honestly.** Kotlin is extracted at bind levels 0-2 (declarations, references, imports, inheritance). It has no receiver-type substrate, so Kotlin hops never carry `RECEIVER_TYPE_MATCH` from receiver narrowing. Calls made through OPERATOR CONVENTIONS are not extracted at all: `a + b` reaching a `private operator fun plus`, and `m[k]` reaching a `private operator fun get`, produce no edge, so such a member can be reported definitely dead while genuinely called. `infix` calls (`a matches b`) ARE extracted. Treat a Kotlin-only `definitely_dead` verdict on an `operator fun` as unproven.
